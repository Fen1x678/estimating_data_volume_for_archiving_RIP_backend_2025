...
from app.serializers import CompressionSerializer
...
def add_compressions():
    ...
    add_compression(3, algorithms, users[0], moderators)
    add_compression(4, algorithms, users[0], moderators)
    add_compression(5, algorithms, users[0], moderators)

    for _ in range(10):
        status = random.randint(2, 5)
        add_compression(status, algorithms, users[0], moderators)
...
def add_compression(status, algorithms, owner, moderators):
    ...
    compression.data_type = random.randint(1, 10)

    compression.owner = owner

    for algorithm in random.sample(list(algorithms), random.randint(1, 3)):
